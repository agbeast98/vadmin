...
import { Activity, CreditCard, Users, Server, Ticket, DollarSign, ShoppingBag, ArrowUp, ArrowDown, Users2 } from 'lucide-react';
import type { Account, Plan, Service, Ticket as TicketType, TicketPriority, TicketStatus } from '@/lib/types';
...
import { PLANS_STORAGE_KEY, SERVICES_STORAGE_KEY, TICKETS_STORAGE_KEY } from '@/lib/constants';
import { subDays, startOfDay, endOfDay, isWithinInterval, startOfToday, format, isSameDay } from 'date-fns';
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      if (user.role === 'admin') {
        ...
        setNewUsersThisWeek(accounts.filter(acc => acc.role === 'user' && new Date(acc.createdAt) >= oneWeekAgo).length);
    ...
    <>
      <div className="grid gap-4 md:grid-cols-2 md:gap-8 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{formatCurrency(revenueToday)} <span className="text-sm text-muted-foreground">تومان</span></div>
            <p className="text-xs text-muted-foreground">درآمد حاصل از فروش امروز</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">تعداد کل سفارش‌های ثبت شده امروز</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">کاربران جدید (هفته)</CardTitle>
            <Users2 className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">+{newUsersThisWeek}</div>
            <p className="text-xs text-muted-foreground">کاربران ثبت‌نام شده در ۷ روز اخیر</p>
          </CardContent>
        </Card>
      </div>
                         ...
                         {urgentTickets.length === 0 ? (
                            ...
                            urgentTickets.map(ticket => (
                                <TableRow key={ticket.id}>
                                    <TableCell>
                                        <Link href={`/dashboard/tickets/${encodeURIComponent(ticket.id)}`} className="font-medium hover:underline">
                                            {ticket.subject}
                                        </Link>
                                    </TableCell>
                                    <TableCell>
